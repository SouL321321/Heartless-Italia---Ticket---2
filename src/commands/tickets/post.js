  ...
  async execute(interaction) {
    ...
    if (!config) {
      return interaction.reply({
        content:
          "Il sistema di ticket non è stato configurato in questo server. Usa `/ticket-config` per configurarlo prima.",
        ephemeral: true,
      });
    }

    // Deferring the reply to make sure interaction isn't acknowledged twice
    await interaction.deferReply({ ephemeral: true });
    ...
    // Use followUp to send the confirmation reply after the embed has been sent
    await interaction.followUp({ content: "Ticket postato!", ephemeral: true });
  },
};
